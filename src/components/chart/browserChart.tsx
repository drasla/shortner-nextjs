...
function BrowserChart({ selectedPanel }: Props) {
    const t = useTranslations();
    const [data, setData] = useState<(BrowserStat & { fill: string })[]>([]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await fetch("/api/stats/browser", {
                    body: JSON.stringify({
                ...
                setData(ChartUtility.applyGradientColors(json.browserStats));
            } catch {
                setData([]);
            } finally {
                setLoading(false);
